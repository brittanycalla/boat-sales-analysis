...
# 2. Examining listing views by country
boat_data %>% 
  group_by(country) %>% 
...
# plotting listing views by currency
boat_data %>%
  group_by(currency) %>% 
...
# 3. Used vs new boat listings
# create top 25% most viewed listings
top_25_perc <- boat_data %>% 
  filter(views > quantile(boat_data$views, prob = .75))

# plot most viewed listings by condition
top_25_perc %>% 
...
# plot most viewed average listings by condition
top_25_perc %>% 
  filter(!is.na(condition)) %>% 
  group_by(condition) %>% 
  summarize(total = n(),
...
# 4. Examining popular boat types
top_25_perc %>%
...
# 5. Do multiple tags lead to more views?
top_25_perc_sep %>% 
  group_by(no_tags) %>% 
  summarize(avg_views = mean(views)) %>% 
